...
_____
### PB -  Powerfull Array
* Problem link 		: [codeforces - 86D](http://codeforces.com/contest/86/problem/D)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/86D.cpp)
* Solution method	: Mo's algorithm, Math
* Compexity			: `O(M*sqrtN)`

#### Explanation
Use Mo's Algorithm to solve the problem. For each query change the `curL` and `curR` and update `rangeSum` each time. And write a formula to calculate `rangeSum`.
```
    formula(number, count)
        return number * (count^2 - (count-1)^2)
    
    // IN MO's Algorithm use this formula to update sum
    // For example..

    L = query.left
    R = query.right

    while curL < L                  // rangeSum decreasing
        number = arr[curL]
        rangeSum -= formula(number, count[number])
        count[number]--
        curL++

    while curL > L                  // rangeSum increasing
        curL--
        number = arr[curL]
        count[number]--
        rangeSum -= formula(number, count[number])
...
_____
### PB -  Triplets
* Problem link 		: [exam - Triplets](https://github.com/rossanoventurini/CompetitiveProgramming/blob/master/Exams/Text14022018.pdf)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/exam_Triplets.cpp)
* Solution method	: Linear
* Compexity			: `O(N^2)`

#### Explanation
start from checking from `2nd` element to `(N-1)'th` element. For each index `i`, count how many element have in range `(1, i)` that smaller than `i'th` element of array.
```
    current = arr[i]
    for j from 1 to i - 1
        if(arr[j] < current)
            count1++
```
And, for each index `i`, count how many element have in range (i+1, N) that greater than `i'th` element of the array.
```
    current = arr[i]
    for j from i + 1 to N
        if(arr[j] > current)
            count2++
```
At the end, number of the triples is equal to sum of the `count1 * count2` 
_____
### PB -  Array Copy
* Problem link 		: [exam - Triplets](https://github.com/rossanoventurini/CompetitiveProgramming/blob/master/Exams/Text23012018.pdf)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/exam_Array_Copy.cpp)
* Solution method	: Segment tree
* Compexity			: `O(M*logN)`

#### Explanation
To solve this problem, we can use update and query methods of the segment tree. We don't need build method, because we have a `B` array and we will change some range of the array `B`. The important case for this problem is uptade method. `B[leftB..leftB+k] = A[leftA..leftA+k]`
```
...
_____
### PB -  X Total Shapes
* Problem link 		: [geeksforgeeks - X Total Shapes](https://practice.geeksforgeeks.org/problems/x-total-shapes/0)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/geeks_X-Total-Shapes.cpp)
* Solution method	: DFS
* Compexity			: `O(N*M)`

#### Explanation
Convert given input two dimensial array. And then start to check `(i,j)` from `1,1` to `N,N`, if `shape[i,j]` is equal to `'X'`, then start `DFS` from this point. And color the visited points each time. In `DFS` function,
for each `(x,y)` coordinate, do `DFS` to non colored `(x,y-1)`, `(x,y+1)`, `(x-1,y)`, `(x+1,y)` points.
```
    result = 0
    for i from 1 to N
        for j from 1 to M
...
_____
### PB -  Bipartite Graph
* Problem link 		: [geeksforgeeks - Bipartite Graph](https://practice.geeksforgeeks.org/problems/bipartite-graph/1)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/geeks_Bipartite-Graph.cpp)
* Solution method	: Graph
* Compexity			: `O(N+M)`

#### Explanation
In Bipartite graph, we color the graph using two colors. It is also possible for cycle graphs, but number of the nodes must be even. It is not possible to color a cycle graph with odd cycle using two colors. Additionally, if there is a self loop, then it can not be bipartite graph.
```
    for i from 1 to N
        for j from 1 to N
            if arr[i][j] == 1
                print "NO"
```
To solve this problem, start to check from `1st` point. Then, find all non-colored adjacent vertices. if destination `v` is not colored, then assign alternate color to this adjacent `v` of `u`.
```
    u = FRONT(Q)
    REMOVE(Q, u)
    for each v in u
...
```
_____
### PB -  Learning Languages
* Problem link 		: [codeforces - 277A](http://codeforces.com/contest/277/problem/A)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/277A.cpp)
* Solution method	: Graph, DFS
* Compexity			: `O(N+M)`

#### Explanation
It is a basic Graph problem, we need to find how many Strongly Connected Components have in this Graph.
```
    DFS(u)
        visited(u)
        for each v in u
...
_____
### PB -  Checkposts
* Problem link 		: [codeforces - 427C](http://codeforces.com/contest/427/problem/C)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/427C.cpp)
* Solution method	: Graph, DFS
* Compexity			: `O(2*(N+M))`
...
```
    DFS(u)
        visited(u)
        ADD(POSTS, u)    // we add all nodes of the connected component to the POSTS 
        for each v in u
            if non visited(v)
                DFS(v)
        ADD(POSTS, u)
    ...
    MAIN
        ......              // code here..
        // we search for lowest cost node in each component
        min_cost = INT_MAX
        for each post in POSTS
            min_cost = MIN(min_cost, cost[post])    // we found the lowest cost
        for each post in POSTS
            if(min_cost == cost[post])
                min_count++             // we found how many node have cost is equal to lowest cost
        ......              // code here..
```
_____
### PB -  MST - Minimum Spanning Tree
* Problem link 		: [spoj - MST](https://www.spoj.com/problems/MST/)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/spoj_MST.cpp)
* Solution method	: Graph, Kruskal
* Compexity			: `O(N*logN)`

#### Explanation
Use Kruskal algorithm to solve this minimum spanning tree problem. Start from the minimum weighted edge to biggest one, and then check if parent of the nodes are different, then union the all parents of the nodes of current edge.
```
    MAIN
        SORT(Arr)
        KRUSKAL()
...
_____
### PB -  Magic Numbers
* Problem link 		: [codeforces - 320A](http://codeforces.com/contest/320/problem/A)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/320A.cpp)
* Solution method	: Greedy, Linear
* Compexity			: `O(N)`

#### Explanation
if in initial string there have these following cases, then we skip to next indexed, else it is not magic number.
1. `str[i] == 1`
2. `str[i] == 4 && str[i-1] == 1`
3. `str[i] == 4 && str[i-1] == 4 && str[i-2] == 1`

_____
### PB -  Wilbur and Array
* Problem link 		: [codeforces - Wilbur and Array](http://codeforces.com/contest/596/problem/B)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/596B.cpp)
* Solution method	: Greedy
* Compexity			: `O(N)`

#### Explanation
We need `|b1|` operations to make `a1` equal to `b1`. Then we need `|b2 - b1|` operations to make `a2` equal to `b2`. In general, to make `ai = bi` we need to make `|bi - bi - 1|` operations, so in total we make `|b1| + |b2 - b1| + |b3 - b2| + ... + |bn - bn - 1|` operations.
```
    for i from 1 to N
        read(x)
        x += v
...
```
_____
### PB -  Alternative Thinking
* Problem link 		: [codeforces - 603A](http://codeforces.com/contest/603/problem/A)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/603A.cpp)
* Solution method	: Greedy
* Compexity			: `O(N)`

#### Explanation
In this problem we can change string in these following ways.
1. for `...000...` string we change it to `...010...`
2. for `...1111...` string we change it to `...1001....`
3. for `...00000...` string we change it to `...01110...`
```
    read str
    from i from 1 to str.length
        if str[i] == str[i-1]
            count++
    result = MIN(result, result - count + 2)
```
_____
### PB -  N meetings in one room
* Problem link 		: [geeksforgeeks - N meetings in one room](https://practice.geeksforgeeks.org/problems/n-meetings-in-one-room/0)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/geeks_N-meetings-in-one-room.cpp)
* Solution method	: Greedy
* Compexity			: `O(N*logN)`

#### Explanation
Firstly, sort all meetings by their finish time. Then start first meeting and search if start time of this meeting is after the finish time of the last meeting. If yes then add it to result, else skip to next one.
```
    arr[0] = Meeting(0, 0, 0)       // Meeting(start, finish, index)
    last_index = 0
    for i from 1 to N
        if arr[i].start > arr[last_index].finish
            ADD(result, index)      // we save index because of sorting process
            last_index = i
```
_____
### PB -  Lexicographically Maximum Subsequence
* Problem link 		: [codeforces - 196A](http://codeforces.com/contest/196/problem/A)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/196A.cpp)
* Solution method	: Greedy
* Compexity			: `O(N)`

#### Explanation
Get a new vector. Add each character to vector, But before adding, look the last element of the the vector, while it is smaller then current character, then remove it, else add this character to vector.
```
    read string
    for each ch in string
        while non empty(v) && v.last < ch   
            remove_back()
        v.add(ch)
```
_____
### PB -  Woodcutters
* Problem link 		: [codeforces - 545C](http://codeforces.com/contest/545/problem/C)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/545C.cpp)
* Solution method	: Greedy
* Compexity			: `O(N)`
...
_____
### PB -  Queue
* Problem link 		: [codeforces - 141C](http://codeforces.com/contest/141/problem/C)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/141C.cpp)
* Solution method	: Greedy
* Compexity			: `O(N^2)`

#### Explanation
Sort all array by the height. Then crate a new queue, and add each person to his place in new queue.
```
    for i from 1 to N
        curr = person[i]
        ADD(Q, curr)
        while x.height < curr.height        // x in Q
            swap(x, curr)                   // exchange the place
            x--
```
_____
### PB -  Longest Common Subsequence
* Problem link 		: [geeksforgeeks - LCS](https://practice.geeksforgeeks.org/problems/longest-common-subsequence/0)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/geeks_LCS.cpp)
* Solution method	: LCS
* Compexity			: `O(N*M)`

#### Explanation
it is basic LCS problem, we need to find the length of the subsequence. So, lets create two dimensial `dp[1.N][1..M]` array, and do these process.
1. `if A[i] == B[j], then dp[i][j] = dp[i-1][j-1] + 1`
2. `else, dp[i][j] = MAX(dp[i][j-1], dp[i-1][j])` 

_____
### PB -  0 - 1 Knapsack Problem
* Problem link 		: [geeksforgeeks - Knapsack Problem](https://practice.geeksforgeeks.org/problems/0-1-knapsack-problem/0)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/geeks_Knapsack-Problem.cpp)
* Solution method	: Knapsack
* Compexity			: `O(2^n)`

#### Explanation
Start from searching in array, If weight of the `x'th` item is more than Knapsack capacity `W`, then this item cannot be included. Otherwise, return the maximum of the values `x'th` included and not included.
```
    if weight[x] > Total
        return Knapsack(x - 1, Total)

    return MAX(
        value[x] + Knapsack(x - 1, Total - weight[x]),
        Knapsack(x - 1, Total)
    )
```
_____
### PB -  Longest Increasing Subsequence
* Problem link 		: [geeksforgeeks - LIS](https://practice.geeksforgeeks.org/problems/longest-increasing-subsequence/0)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/geeks_LIS-(dp).cpp)
* Solution method	: LIC
* Compexity			: `O(n^2)`

#### Explanation
For each element before the `i'th` element of the array, check if it is smaller or not. if it is, then the length of the longest increasing subsequence for index `i` is equal to `max(lic[i], lic[j] + 1)`.
```
    lic[1] = 1
    for i from 2 to N
        for j from 1 to i
...
``` 
_____
### PB - Subset Sum Problem
* Problem link 		: [geeksforgeeks - Subset Sum](https://practice.geeksforgeeks.org/problems/subset-sum-problem/0)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/geeks_Subset-Sum-Problem.cpp)
* Solution method	: Knapsack
* Compexity			: `O(N*sum)`

#### Explanation
Firstly find sum of the all elements, then do knapsack with total weight `sum/2`
_____
...
```
_____
### PB -  Longest Prefix Suffix
* Problem link 		: [geeksforgeeks - Longest Prefix Suffix](https://practice.geeksforgeeks.org/problems/longest-prefix-suffix/0)
* Solution code 	: [cpp](https://github.com/azecoder/CPAC/blob/master/HomeWorks/geeks_Longest-Prefix-Suffix.cpp)
* Solution method	: KMP 
* Compexity			: `O(N)`

#### Explanation
Create prefix array of the Knuth Morris Pratt algorithm. And get the maximum value of the prefix array as a result.
```
    ind = 0, i = 1
    while i < S.length
        if S[i] == S[ind]
